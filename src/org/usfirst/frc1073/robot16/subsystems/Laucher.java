...
public class Laucher extends Subsystem implements PIDSubsystem {
	...
	private final Encoder distance = RobotMap.laucherlaucherLimitEncoder;

	private final boolean open = false;
	private final boolean closed = true;
	
	private final double LAUCHER_MAX = 0.0;
	private final double LAUCHER_MIN = 4.0;
	private final double INCHES_PER_PULSE = .0036363636364;

	// Put methods for controlling this subsystem
	// here. Call these from Commands.

	public enum laucherState { readyToLauch, 
							   emptyMiddle,
							   closedForwards,
							   emptyForwards,
							   emptyBackwards,
	...
	public Laucher() {
		current = laucherState.emptyMiddle; // Robot starts emptyMiddle
		pullBackMotor.setInverted(Robot.invertLaucher);
		elevationMotor.setInverted(Robot.invertLaucherElevation);
		distance.setDistancePerPulse(INCHES_PER_PULSE);
	}
	 ...
	 *
	 *************************************/
	public void setCurrentState(laucherState newState) {
		current = newState;
	}
	...
	public void setLaucherPos(double dist) {
		//Distance given by Derek. 137.5 revs for 1 inch of travel
		if(distance.getDistance() < dist && !isMaxed()) {
			driveLaucherMotorBackwards();
		}
		else if(distance.getDistance() > dist && !isMin()) {
	...
	private boolean isMaxed() {		
		if(distance.getDistance() >= LAUCHER_MAX)
			return true;
		else
			return false;
	}
	...
	private boolean isMin() {
		if(distance.getDistance() <= LAUCHER_MIN)
			return true;
		else
			return false;
	}
	...
	public void initDefaultCommand() {
		...
		{
			SmartDashboard.putString("Error", "Launcher is locked. Cant move.");
		}
